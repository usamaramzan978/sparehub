...
@section('content')
    ...
    <x-breadcrumb title="{{ __('Edit Vendor Payment') }}" :items="$breadcrumbs">
        <x-slot:actions>
    ...
    <div class="card custom-card border-0 shadow-sm h-100">
        <div class="card-body">
            ...
            <form method="POST" action="{{ route('tenant.vendor-payments.update', $vendorPayment) }}">
                @csrf
                @method('PUT')
                <div class="row">
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <select name="vendor_id" id="vendor_id"
                            ...
                            @foreach ($vendors as $vendor)
                                <option value="{{ $vendor->id }}" @selected(old('vendor_id', $vendorPayment->vendor_id) === $vendor->id)>
                                    {{ $vendor->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <select name="purchase_id" id="purchase_id"
                            ...
                            @foreach ($purchases as $purchase)
                                <option value="{{ $purchase->id }}" @selected(old('purchase_id', $vendorPayment->purchase_id) === $purchase->id)>
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="number" step="0.01" min="0.01" name="amount" id="amount"
                            ...
                            value="{{ old('amount', (string) $vendorPayment->amount) }}" required>
                        @error('amount')
                            <span class="invalid-feedback d-block">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" name="reference_no" id="reference_no"
                            ...
                            value="{{ old('reference_no', $vendorPayment->reference_no) }}">
                        @error('reference_no')
                            <span class="invalid-feedback d-block">{{ $message }}</span>
                        @enderror
                    </div>
                    <div class="col-md-12 mb-3">
                        ...
                        <textarea name="notes" id="notes" rows="2" class="form-control @error('notes') is-invalid @enderror">{{ old('notes', $vendorPayment->notes) }}</textarea>
                        @error('notes')
                            <span class="invalid-feedback d-block">{{ $message }}</span>
                        @enderror
                    </div>
