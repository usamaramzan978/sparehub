...
@section('content')
    ...
    <div class="card">
        <div class="card-body">
            ...
            <form method="POST" action="{{ route('tenant.purchase-returns.update', $purchaseReturn) }}">
                @csrf
                @method('PUT')
                <div class="row">
                    <div class="col-md-3 mb-3">
                        ...
                        <input type="text" name="return_no" id="return_no"
                            ...
                            value="{{ old('return_no', $purchaseReturn->return_no) }}" required>
                        @error('return_no')
                    ...
                    <div class="col-md-3 mb-3">
                        ...
                        <select name="vendor_id" id="vendor_id"
                            ...
                            @foreach ($vendors as $vendor)
                                <option value="{{ $vendor->id }}" @selected(old('vendor_id', $purchaseReturn->vendor_id) === $vendor->id)>
                                    {{ $vendor->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-3 mb-3">
                        ...
                        <select name="purchase_id" id="purchase_id"
                            ...
                            @foreach ($purchases as $purchase)
                                <option value="{{ $purchase->id }}" @selected(old('purchase_id', $purchaseReturn->purchase_id) === $purchase->id)>
                    ...
                    <div class="col-md-3 mb-3">
                        ...
                        <select name="status" id="status"
                            ...
                            @foreach ($statuses as $status)
                                <option value="{{ $status->value }}" @selected(old('status', $purchaseReturn->status->value) === $status->value)>
                    ...
                    <div class="col-md-3 mb-3">
                        ...
                        <input type="number" step="0.01" name="sub_total" id="sub_total"
                            ...
                            value="{{ old('sub_total', (string) $purchaseReturn->sub_total) }}">
                        @error('sub_total')
                            <span class="invalid-feedback d-block">{{ $message }}</span>
                        @enderror
                    </div>
                    <div class="col-md-3 mb-3">
                        ...
                        <input type="number" step="0.01" name="tax_total" id="tax_total"
                            ...
                            value="{{ old('tax_total', (string) $purchaseReturn->tax_total) }}">
                        @error('tax_total')
                            <span class="invalid-feedback d-block">{{ $message }}</span>
                        @enderror
                    </div>
                    <div class="col-md-3 mb-3">
                        ...
                        <input type="number" step="0.01" name="grand_total" id="grand_total"
                            ...
                            value="{{ old('grand_total', (string) $purchaseReturn->grand_total) }}">
                        @error('grand_total')
                            <span class="invalid-feedback d-block">{{ $message }}</span>
                        @enderror
                    </div>
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="datetime-local" name="posted_at" id="posted_at"
                            ...
                            value="{{ old('posted_at', $purchaseReturn->posted_at?->format('Y-m-d\TH:i')) }}">
                        @error('posted_at')
                            <span class="invalid-feedback d-block">{{ $message }}</span>
                        @enderror
                    </div>
                    <div class="col-md-12 mb-3">
                        ...
                        <textarea name="notes" id="notes" rows="2" class="form-control @error('notes') is-invalid @enderror">{{ old('notes', $purchaseReturn->notes) }}</textarea>
                        @error('notes')
                            <span class="invalid-feedback d-block">{{ $message }}</span>
                        @enderror
                    </div>
