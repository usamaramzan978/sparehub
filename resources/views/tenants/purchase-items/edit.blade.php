...
@section('content')
    ...
    <x-breadcrumb title="{{ __('Edit Purchase Item') }}" :items="$breadcrumbs">
        <x-slot:actions>
    ...
    <div class="card">
        <div class="card-body">
            ...
            <form method="POST" action="{{ route('tenant.purchase-items.update', $purchaseItem) }}">
                @csrf
                @method('PUT')
                <div class="row">
                    <div class="col-md-4 mb-3">
                        ...
                        <select name="purchase_id" id="purchase_id"
                            ...
                            @foreach ($purchases as $purchase)
                                <option value="{{ $purchase->id }}" @selected(old('purchase_id', $purchaseItem->purchase_id) === $purchase->id)>
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <select name="product_id" id="product_id"
                            ...
                            @foreach ($products as $product)
                                <option value="{{ $product->id }}" @selected(old('product_id', $purchaseItem->product_id) === $product->id)>
                                    {{ $product->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <select name="tax_id" id="tax_id"
                            ...
                            @foreach ($taxes as $tax)
                                <option value="{{ $tax->id }}" @selected(old('tax_id', $purchaseItem->tax_id) === $tax->id)>
                                    {{ $tax->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-2 mb-3">
                        ...
                        <input type="number" step="0.001" min="0.001" name="qty" id="qty"
                            class="form-control @error('qty') is-invalid @enderror" value="{{ old('qty', (string) $purchaseItem->qty) }}"
                            required>
                        @error('qty')
                            <span class="invalid-feedback d-block">{{ $message }}</span>
                        @enderror
                    ...
                    <div class="col-md-2 mb-3">
                        ...
                        <input type="number" step="0.001" min="0" name="received_qty" id="received_qty"
                            ...
                            value="{{ old('received_qty', (string) $purchaseItem->received_qty) }}">
                        @error('received_qty')
                            <span class="invalid-feedback d-block">{{ $message }}</span>
                        @enderror
                    </div>
                    <div class="col-md-2 mb-3">
                        ...
                        <input type="number" step="0.01" min="0" name="unit_cost" id="unit_cost"
                            ...
                            value="{{ old('unit_cost', (string) $purchaseItem->unit_cost) }}" required>
                        @error('unit_cost')
                            <span class="invalid-feedback d-block">{{ $message }}</span>
                        @enderror
                    </div>
                    <div class="col-md-2 mb-3">
                        ...
                        <input type="number" step="0.01" min="0" name="discount_amount" id="discount_amount"
                            ...
                            value="{{ old('discount_amount', (string) $purchaseItem->discount_amount) }}">
                    ...
                    <div class="col-md-2 mb-3">
                        ...
                        <input type="number" step="0.01" min="0" name="tax_amount" id="tax_amount"
                            ...
                            value="{{ old('tax_amount', (string) $purchaseItem->tax_amount) }}">
                        @error('tax_amount')
                            <span class="invalid-feedback d-block">{{ $message }}</span>
                        @enderror
                    </div>
                    <div class="col-md-12 mb-3">
                        ...
                        <textarea name="remarks" id="remarks" rows="2" class="form-control @error('remarks') is-invalid @enderror">{{ old('remarks', $purchaseItem->remarks) }}</textarea>
                        @error('remarks')
                            <span class="invalid-feedback d-block">{{ $message }}</span>
                        @enderror
                    </div>
