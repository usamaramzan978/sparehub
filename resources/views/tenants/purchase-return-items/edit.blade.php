...
@section('content')
    ...
    <div class="card">
        <div class="card-body">
            ...
            <form method="POST" action="{{ route('tenant.purchase-return-items.update', $purchaseReturnItem) }}">
                @csrf
                @method('PUT')
                <div class="row">
                    <div class="col-md-4 mb-3">
                        ...
                        <select name="purchase_return_id" id="purchase_return_id"
                            ...
                            @foreach ($purchaseReturns as $purchaseReturn)
                                <option value="{{ $purchaseReturn->id }}" @selected(old('purchase_return_id', $purchaseReturnItem->purchase_return_id) === $purchaseReturn->id)>
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <select name="product_id" id="product_id"
                            ...
                            @foreach ($products as $product)
                                <option value="{{ $product->id }}" @selected(old('product_id', $purchaseReturnItem->product_id) === $product->id)>
                                    {{ $product->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <select name="tax_id" id="tax_id"
                            ...
                            @foreach ($taxes as $tax)
                                <option value="{{ $tax->id }}" @selected(old('tax_id', $purchaseReturnItem->tax_id) === $tax->id)>
                                    {{ $tax->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-2 mb-3">
                        ...
                        <input type="number" step="0.001" min="0.001" name="qty" id="qty"
                            ...
                            value="{{ old('qty', (string) $purchaseReturnItem->qty) }}" required>
                        @error('qty')
                            <span class="invalid-feedback d-block">{{ $message }}</span>
                        @enderror
                    </div>
                    <div class="col-md-2 mb-3">
                        ...
                        <input type="number" step="0.01" min="0" name="unit_cost" id="unit_cost"
                            ...
                            value="{{ old('unit_cost', (string) $purchaseReturnItem->unit_cost) }}" required>
                        @error('unit_cost')
                            <span class="invalid-feedback d-block">{{ $message }}</span>
                        @enderror
                    </div>
                    <div class="col-md-2 mb-3">
                        ...
                        <input type="number" step="0.01" min="0" name="tax_amount" id="tax_amount"
                            ...
                            value="{{ old('tax_amount', (string) $purchaseReturnItem->tax_amount) }}">
                        @error('tax_amount')
                            <span class="invalid-feedback d-block">{{ $message }}</span>
                        @enderror
                    </div>
                    <div class="col-md-12 mb-3">
                        ...
                        <textarea name="remarks" id="remarks" rows="2" class="form-control @error('remarks') is-invalid @enderror">{{ old('remarks', $purchaseReturnItem->remarks) }}</textarea>
                        @error('remarks')
                            <span class="invalid-feedback d-block">{{ $message }}</span>
                        @enderror
                    </div>
