...
@section('content')
    ...
    <x-breadcrumb title="{{ __('Edit Job Card') }}" :items="$breadcrumbs">
        <x-slot:actions>
    ...
    <div class="card custom-card border-0 shadow-sm h-100">
        <div class="card-body">
            ...
            <form method="POST" action="{{ route('tenant.job-cards.update', $jobCard) }}">
                @csrf
                @method('PUT')
                <div class="row">
                    <div class="col-md-3 mb-3">
                        ...
                        <input type="text" name="job_no" id="job_no"
                            class="form-control @error('job_no') is-invalid @enderror"
                            value="{{ old('job_no', $jobCard->job_no) }}" required>
                        @error('job_no')
                    ...
                    <div class="col-md-3 mb-3">
                        ...
                        <select name="status" id="status"
                            ...
                            @foreach ($statuses as $status)
                                <option value="{{ $status->value }}" @selected(old('status', $jobCard->status->value) === $status->value)>
                    ...
                    <div class="col-md-3 mb-3">
                        ...
                        <input type="number" step="0.001" min="0" name="meter_reading" id="meter_reading"
                            ...
                            value="{{ old('meter_reading', (string) $jobCard->meter_reading) }}">
                        @error('meter_reading')
                            <span class="invalid-feedback d-block">{{ $message }}</span>
                        @enderror
                    </div>
                    <div class="col-md-3 mb-3">
                        ...
                        <input type="number" step="0.001" min="0" name="next_reading" id="next_reading"
                            ...
                            value="{{ old('next_reading', (string) $jobCard->next_reading) }}">
                        @error('next_reading')
                            <span class="invalid-feedback d-block">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-2 mb-3">
                        ...
                        <input type="number" min="0" name="total_visits" id="total_visits"
                            ...
                            value="{{ old('total_visits', $jobCard->total_visits) }}">
                        @error('total_visits')
                            <span class="invalid-feedback d-block">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-12 mb-3">
                        ...
                        <textarea name="remarks" id="remarks" rows="2" class="form-control @error('remarks') is-invalid @enderror">{{ old('remarks', $jobCard->remarks) }}</textarea>
                        @error('remarks')
                            <span class="invalid-feedback d-block">{{ $message }}</span>
                        @enderror
                    </div>
