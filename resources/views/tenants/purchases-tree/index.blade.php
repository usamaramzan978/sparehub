...
@section('content')
            ...
            <form method="GET" action="{{ route('tenant.purchases-tree.index') }}" class="row g-2">
                ...
                <div class="col-md-3">
                    ...
                    <input type="text" class="form-control" id="search" name="search" value="{{ $filters['search'] }}"
                        placeholder="{{ __('Purchase, vendor, product') }}">
                </div>
                <div class="col-12 d-flex justify-content-end gap-2">
                    <button type="submit" class="btn btn-primary">{{ __('Filter') }}</button>
                    <a href="{{ route('tenant.purchases-tree.index') }}" class="btn btn-outline-secondary">{{ __('Reset') }}</a>
                </div>
            </form>
        </div>
    </div>
    ...
    <div class="row g-3 mb-3">
        <div class="col-12 col-md-4"><div class="card"><div class="card-body"><div class="text-muted small">{{ __('Vendors') }}</div><h5 class="mb-0">{{ $summary['vendors_count'] }}</h5></div></div></div>
        <div class="col-12 col-md-4"><div class="card"><div class="card-body"><div class="text-muted small">{{ __('Purchases') }}</div><h5 class="mb-0">{{ $summary['purchases_count'] }}</h5></div></div></div>
        ...
        <div class="col-12 col-md-4"><div class="card"><div class="card-body"><div class="text-muted small">{{ __('Total Purchases') }}</div><h5 class="mb-0">{{ $money($summary['grand_total']) }}</h5></div></div></div>
