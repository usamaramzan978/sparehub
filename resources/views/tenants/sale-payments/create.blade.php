...
@section('content')
    ...
    <x-breadcrumb title="{{ __('Create Sale Payment') }}" :items="$breadcrumbs">
        <x-slot:actions>
    ...
    <div class="card custom-card border-0 shadow-sm h-100">
        <div class="card-body">
            ...
            <form method="POST" action="{{ route('tenant.sale-payments.store') }}">
                @csrf
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="received_by" id="received_by"
                            ...
                            @foreach ($receivers as $receiver)
                                <option value="{{ $receiver->id }}" @selected(old('received_by') === $receiver->id)>
                                    {{ $receiver->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="datetime-local" name="paid_at" id="paid_at"
                            ...
                            value="{{ old('paid_at', \App\Support\TenantDateTime::format(now(), 'Y-m-d\\TH:i', '')) }}" required>
                        @error('paid_at')
                            <span class="invalid-feedback d-block">{{ $message }}</span>
                        @enderror
                    </div>
